...
def ising_energy_plot(energies, L, show_plot=True, save_plot=False, plot_file_Name=None):
    ...
    plt.hist(energies, bins=L**3+1, label='Histograma energías\nIsing $L\\times L=%d$'%(L*L))
    plt.xlabel('$E$')
...
def energy_data_to_frequencies(energy_data_file_name):
    ...
    energy_data = energy_data.to_numpy(dtype=int)
    energy_data = energy_data.transpose()
    energy_data = energy_data.tolist()[0]
    energy_omegas = dict(collections.Counter(energy_data))
    ...
    energies, frequencies = np.array(energy_omegas[0]), np.array(energy_omegas[1])
    return energies, frequencies
