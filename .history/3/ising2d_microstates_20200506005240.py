...
def partition(energies, L, beta=4, beta_max=None, N_values=None, read_data=False, energy_data_file_name=None):
    if read_data:
        energies = read_energy_data(energy_data_file_name)
    energies, omegas = energies_to_frequencies(energies)

    if not beta_max:
        return sum(omegas * np.exp(- beta * energies))
    else:
        beta_array = np.linspace(beta, beta_max, N_values)
        Z.append(sum(omegas * np.exp(- beta * energies)))
        return Z
