...
def ising_energy_plot(energies, L, show_plot=True, save_plot=False, plot_file_Name=None):
    ...
    if save_plot:
        if not plot_file_Name:
            plot_file_Name = 'ising-energy-plot-L_%d.eps'%(L)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        plot_file_Name = script_dir + '/' + plot_file_Name
        plt.savefig(plot_file_Name)
    if show_plot:
...
print('All microstates, each in a single row:',pd.DataFrame(microstates))
...
print('One random microstate as a 2D grid:', pd.DataFrame(microstate_rand.reshape((L,L))))
