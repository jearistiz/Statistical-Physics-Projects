...
def save_paths_x_csv(paths, file_name2, relevant_info, print_data=True):
    """
    ...
    paths_x_data = {}
    script_dir = os.path.dirname(os.path.abspath(__file__))
    filename = script_dir + '/' + file_name2
...
csv_file_name =     'pi_x-pi-%s-N_path_%d-N_iter_%d-beta_fin_%.3f-delta_%.3f-append_every_%d.csv'\
                    %(potential_string,N_path,N_iter,beta,delta,append_every)

relevant_info = [   'All paths generated by path_integral_naive_sampling.py each row is a path',
                    '%s    N_path = %d    N_iter = %d    beta_fin = %.3f    delta = %.3f'\
                    %(potential_string, N_path, N_iter, beta, delta)]

save_paths_x_csv(pathss_x, csv_file_name, relevant_info, print_data=True)

plot_file_name =    'pi_x-pi-plot-%s-N_path_%d-N_iter_%d-beta_fin_%.3f-delta_%.3f-append_every_%d-x_max_%.3f-theory_%d-ms_%d-path_%d-compare_%d-complete_path-%d.eps'\
                    %(potential_string,N_path,N_iter,beta,delta,append_every,x_max,show_theory,show_matrix_squaring,show_path,show_compare_hist,show_complete_path_hist)

figures_fn( pathss_x, beta = beta , N_plot = N_plot, x_max = x_max, N_iter=N_iter, 
            append_every=append_every, N_beta_ticks = 10, msq_file=msq_file,
