...
def Z_several_values(   temp_min=1./10, temp_max=1/2., N_temp=10, save_Z_csv=True,
    ...
    if save_Z_csv == True:
        if Z_file_name is None:
            script_dir = os.path.dirname(os.path.abspath(__file__)) #path completa para este script
            Z_file_name =   'Z-ms-%s-beta_max_%.3f-beta_min_%.3f-'%(1./temp_min,1./temp_max) + \
                            'N_temp_%d-x_max_%.3f-nx_%d-'%(N_temp,potential_string,x_max,nx,) +\
                            'N_iter_%d.csv'%(N_iter)
    ...
    if plot_energy:
        ...
        if save_plot_E:
            if E_plot_name is None:
                ...
                E_plot_name='E-ms-%s-beta_max_%.3f-beta_min_%.3f-'%(1./temp_min,1./temp_max) + \
                            'N_temp_%d-x_max_%.3f-nx_%d-'%(N_temp,potential_string,x_max,nx) + \
                            'N_iter_%d.eps'%(N_iter)
...
# Z_file_name = 'Z-ms-%s-beta_max_%.3f-beta_min_%.3f-N_temp_%d-x_max_%.3f-nx_%d-N_iter_%d.csv' \
#                             %(1./temp_min,1./temp_max,N_temp,potential_string,x_max,nx,N_iter)
# Z_file_name = script_dir + '/' + Z_file_name
if calculate_avg_energy:
    average_energy( read_Z_data, generate_Z_data, Z_file_name,
                        plot_energy, save_plot_E, show_plot_E, 
                        E_plot_name,
                        temp_min, temp_max, N_temp, save_Z_csv,
                        relevant_info_Z, print_Z_data, x_max, nx, N_iter, potential,
                        potential_string, print_steps, save_pi_x_data, pi_x_file_name,
                        relevant_info_pi_x, plot, save_plot, show_plot   )
