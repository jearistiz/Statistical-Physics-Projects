...
def save_rho_csv(grid_x, weights, file_name, relevant_info, print_data=True):
    ...
    with open(file_name,mode='w') as rho_csv:
        rho_csv.write('hello world')#relevant_info)
    rho_csv.close()
...
relevant_info = u'# %s   x_max = %.3f   nx = %d   N_beta = %d   beta_fin = %.3f \n'%(potential_string,x_max,nx,N_beta,beta_fin)
save_rho_csv(grid_x, weights, file_name, relevant_info, print_data=0)
