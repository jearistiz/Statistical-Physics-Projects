...
def optimization(   beta_fin=4, x_max=5, potential=harmonic_potential, 
                    ...
                    save_opt_data = False, opt_data_file_name = None, plot = True, save_plot = True):
    """
    """
    ...
    if save_opt_data:
        if opt_data_file_name is None:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            opt_data_file_name = '/pi_x-ms-opt-%s-beta_fin_%.3f-x_max_%.3f-nx_min_%d-nx_min_%d-nx_sampling_%d-N_iter_min_%d-N_iter_max_%d.csv'%(potential_string, beta_fin, x_max)
            beta_fin=4, x_max=5, potential=harmonic_potential, 
                    potential_string='harmonic_potential',
                    nx_min=50, nx_max=1000, nx_sampling=10, N_iter_min=1, N_iter_max=20,
            opt_data_file_name = script_dir + opt_data_file_name
            save_csv(error, dx_grid, beta_ini_grid,)
