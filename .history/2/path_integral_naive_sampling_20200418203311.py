...
def figures_fn( pathss_x, beta = 4 , N_plot = 201, x_max = 3, N_iter=int(1e5), append_every=1,
    ...
    if show_path:
        ...
        #################################################
        # TERMINAR ESTA PARTE Y ARREGLAR NOMBRES DE PLOT SEGÚN SI SE HA MOSTRADO O NO LAS DIFERENTES OPCIONES
        #################################################   
        elif show_theory and not show_matrix_squaring:
            leg = lns1 + [lns3[2][0]] + lns4
        elif not show_theory and show_matrix_squaring:
            leg = lns2 + [lns3[2][0]] + lns4
        elif not show_theory and  not show_matrix_squaring:
            leg = [lns3[2][0]] + lns4
...
figures_fn( pathss_x, beta = beta , N_plot = N_plot, x_max = x_max, N_iter=N_iter, 
            ...
            show_path=0, save_plot=True, show_plot=True)