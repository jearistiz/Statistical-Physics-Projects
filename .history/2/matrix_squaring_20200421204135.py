...
def optimization(beta_fin=4, x_max=5, potential=harmonic_potential, potential_string='harmonic_potential',
                 nx_min=50, nx_max=1000, N_iter_min=1, N_iter_max=20, save_opt_data = True, plot = True, save_plot = True):
    """
    """
    ...
    for N_iter in range(N_iter_max,N_iter_min-1,-1):
        ...
        for nx in range(nx_max,nx_min,-2):
            ...
            if error_comp_theo==float('nan') or error_comp_theo==float("inf"):
                row.append(1e10)
            else:
                row.append(error_comp_theo)
        try:
            nan_value = max(error)+0.5*np.abs(max(error)-min(error))
        except:
            nan_value = 0
        error.append(row, nan = nan_value, posinf=nan_value, neginf = nan_value)
    print(error)        
...
N_iter_max = 20
t_0 = time()
error, dx_grid, beta_ini_grid = optimization(beta_fin,x_max,potential,potential_string,nx_min,nx_max,N_iter_min,N_iter_max)
t_0 = time()
print(  '----------------------------------------------------------------------------------\n' +
