...
def optimization(beta_fin=4, x_max=5, nx_min=50, nx_max=1000, N_iter_min=1, N_iter_max=20):
    ...
    if nx_max%2==1:
        nx_max += 1
    dx_grid = [2*x_max/(nx-1) for nx in range(nx_min,nx_max,2)]
    beta_ini_grid = [beta_fin * 2**(-N_iter) for N_iter in range(N_iter_min,N_iter_max+1)]
    for N_iter in range(N_iter_min,N_iter_max+1):
        for nx in range(nx_min,nx_max,2):
            rho,trace_rho,grid_x = \
                run_pi_x_sq_trotter( x_max, nx, N_iter, beta_fin, potential, potential_string,
