...
def Z_several_values(temp_min=1./10, temp_max=1/2., N_temp=10, save_Z_csv=True,
    ...
    Recibe:
        temp_min: float         -> Z se calculará para valores de beta en (1/temp_min,1/temp_max)
                                   con N_temp valores igualmente espaciados. 
        temp_max: float.
        N_temp: int.
        save_Z_csv: bool        -> decide si guarda valores calculados en archivo CSV.
        Z_file_name: str        -> nombre del archivo en el que se guardan datos de Z.
...
def optimization(generate_opt_data=True, read_opt_data=False, beta_fin=4, x_max=5, 
    ...
    Recibe:
        ...
        nx_min: int             ->  se relaciona  con dx = 2*x_max/(nx-1)
        nx_max: int             ->
        nx_sampling: int        ->  se usa para generar lista range(nx_max,nx_min,-1*nx_sampling)
        N_iter_min: int         ->  
        N_iter_max: int         ->  se relaciona con beta_ini = beta_fin **(-N_iter). Se gereran
                                    valores de N_iter con range(N_iter_max,N_iter_min-1,-1)
        save_opt_data: bool     ->
        opt_data_file_name: str ->
        plot: bool              ->
        show_plot: bool         ->
        save_plot: bool         ->
        opt_plot_file_name: str ->
    """
