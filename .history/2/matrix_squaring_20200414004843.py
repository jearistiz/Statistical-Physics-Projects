...
def density_matrix_squaring(rho, grid_x, N_iter = 1, beta_ini = 1, print_steps=True):
    ...
    # Cálculo del valor de beta_fin según valores beta_ini t N_iter dados como input
...
def save_pi_x_csv(grid_x, x_weights, file_name, relevant_info, print_data=True):
    ...
    Devuelve:
        pi_x_data: pd.DataFrame             ->  valores de pi(x;beta) para x en grid_x en formato
                                                "pandas".
    """
    # Almacena datos de probabilifad en diccionario: grid_x para posiciones y x_weights para valores
...
def run_pi_x_sq_trotter(x_max=5., nx=201, N_iter=7, beta_fin=4, potential=harmonic_potential,
    ...
    """
    Uso:    corre algoritmo matrix squaring iterativamente (N_iter veces) usando como input
            una matriz densidad en aproximación de Trotter a temperatura inversa  
            beta_ini = beta_fin * 2**(-N_iter) para potencial dado por potential;
            guarda datos de pi(x;beta) vs. x en archivo de texto y grafica pi(x;beta) 
            comparándolo con teoría para el oscilador armónico cuántico.
