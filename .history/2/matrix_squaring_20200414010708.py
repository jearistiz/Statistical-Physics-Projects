...
def density_matrix_squaring(rho, grid_x, N_iter = 1, beta_ini = 1, print_steps=True):
    ...
    Uso:    devuelve matriz densidad luego de aplicarle algoritmo matrix squaring N_iter veces.
            en la primera iteración se usa matriz de densidad dada por el input rho; en las  
            siguientes iteraciones se usa matriz densidad generada por la iteración 
            inmediatamente anterior. El sistema asociado a la matriz densidad obtenida (al final
            de aplicar el algoritmo) está a temperatura inversa beta_fin = beta_ini * 2**(N_iter).
...
def run_pi_x_sq_trotter(x_max=5., nx=201, N_iter=7, beta_fin=4, potential=harmonic_potential,
    ...
    Uso:    corre algoritmo matrix squaring iterativamente (N_iter veces) en la primera
            ...
            inmediatamente anterior. Esta función además guarda datos de pi(x;beta) vs. x
