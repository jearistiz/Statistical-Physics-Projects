...
def rho_trotter(x_max = 5., nx = 101, beta=1, potential=harmonic_potential):
    ...
    rho = np.array([ [ rho_free(x , xp,beta) * np.exp(-0.5*beta*(potential(x)+potential(xp))) for x in grid] for xp in grid])
    return rho, grid_x
...
rho = rho_trotter(beta_ini/N_beta,potential=harmonic_potential)
