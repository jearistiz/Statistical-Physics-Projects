...
\begin{equation}
	\pi^{(Q)(x;\beta)} = \rho\left(x, x ; \beta \right).
\end{equation}

Para encontrar esto usamos el siguiente algoritmo, que está basado en la idea de la integral de camino. En primer lugar proponemos un camino inicializado en cero $X = [x_1, x_2, \dots, x_{N-1} = 0]$ (se deben tomar condiciones de frontera periódicas ya que $x = x^\prime$) recordemos que este camino tiene asociada una amplitud de probabilidad igual a  $\rho (x,x_1; \beta/N) \rho (x_1,x_2; \beta/N) \cdots \rho (x_{N-1},x^{\prime}; \beta/N)$. Posteriormente escogemos un $x_i$ al azar y proponemos un nuevo valor $x_{i\_\mathrm{new}} = x_i + \delta x$ donde $\delta_x \sim U(-\delta,\delta)$, es decir, es un número aleatoro distribuído uniformemente entre $-\delta$ y $\delta$. El nuevo camino propuesto $X_\mathrm{new}=[x_1,\,\,x_2,\,\,\dots,\,\,x_{i\_\mathrm{new}},\,\,\dots,\,\,x_{N-1}]$ tiene asociada una amplitud de probabilidad $\rho (x,x_1; \beta/N) \rho (x_1,x_2; \beta/N) \cdots \rho (x_{i-1},x_{i\_\mathrm{new}}; \beta/N) \rho (x_{i\_\mathrm{new}},x_{i+1}; \beta/N) \cdots \rho (x_{N-1},x^{\prime}; \beta/N)$. De este modo, podemos usar la probabilidad de aceptación del algoritmo Metrópolis para decidir si aceptamos o no el nuevo camino. Dicha probabilidad está dada por
\begin{equation}
p(X \rightarrow X_\mathrm{new}) =	\min(1,\frac{\rho (x_{i-1},x_{i\_\mathrm{new}}; \beta/N) \rho (x_{i\_\mathrm{new}},x_{i+1}; \beta/N) }{\rho (x_{i-1},x_i; \beta/N) \rho (x_i,x_{i+1}; \beta/N)})
\end{equation} 
Este proceso se realiza de forma iterativa $N_\mathrm{iter}$ veces.
En cada iteración se debe guardar el camino aceptado. Posteriormente, construyendo el histograma de los valores de posiciones $x_i$ o de los valores de posiciones de todos los caminos unidos $x_0 \mathrm{U} x_1 \mathrm{U} \cdots \mathrm{U} x_{N-1}$  obtendremos 
