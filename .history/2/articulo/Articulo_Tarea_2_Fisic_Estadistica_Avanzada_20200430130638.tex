...
Para encontrar esto usamos el siguiente algoritmo, que está basado en la idea de la integral de camino. En primer lugar proponemos un camino inicializado en cero $x_1, x_2, x_{N-1} = 0$. Posteriormente escogemos un $x_i$ al azar y proponemos un nuevo valor $x_{i\_new} = x_i + \delta x$ donde $\delta_x \sim U(-\delta,\delta)$, es decir, es un número aleatoro distribuído uniformemente entre $-\delta$ y $\delta$.
A la lista
