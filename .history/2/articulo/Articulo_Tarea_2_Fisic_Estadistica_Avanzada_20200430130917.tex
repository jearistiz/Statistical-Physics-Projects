...
Para encontrar esto usamos el siguiente algoritmo, que está basado en la idea de la integral de camino. En primer lugar proponemos un camino inicializado en cero $x_1, x_2, \dots, x_{N-1} = 0$ (se deben tomar condiciones de frontera periódicas ya que $x = x^\prime$) recordemos que este camino tiene asociada una amplitud de probabilidad igual a  $\rho (x,x_1; \beta/N) \rho (x_1,x_2; \beta/N) \rho (x_{N-1},x^{\prime}; \beta/N)$. Posteriormente escogemos un $x_i$ al azar y proponemos un nuevo valor $x_{i\_new} = x_i + \delta x$ donde $\delta_x \sim U(-\delta,\delta)$, es decir, es un número aleatoro distribuído uniformemente entre $-\delta$ y $\delta$. El nuevo camino propuesto $x_i$
