...
\begin{equation}
	\hat{\rho} = e^{-\beta\hat{H}/N} \cdot e^{-\beta\hat{H}/N}  \cdots e^{-\beta\hat{H}/N} \rightarrow \text{$N$ veces en total}. \label{eq:convolution}
\end{equation}

Por otro lado, en estos algoritmos también usamos la conocida aproximación de Trotter para altas temperaturas de la matriz densidad \hl{CITAAAAAAAAA}
\begin{equation}
...
En este caso usamos la ecuación (\ref{eq:convolution}) con $N=2$ para construir, a partir de dos matrices densidad a temperatura $2/\beta$, una matriz densidad a temperatura $1/\beta$
