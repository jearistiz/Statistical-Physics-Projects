...
\begin{figure}[!ht]
	...
	\caption{Error relativo cuantificado por (\ref{eq:Msq-err-comp}) para $\beta_\mathrm{fin}=4$. Notamos que cuanto más pequeño sea $\beta_\mathrm{ini}$, la discretización $dx$ debe disminuir también para evitar que el error computacional sea grande.}
...
Para obtener $\pi^{(Q)}_\mathrm{comp}(x ; \beta_\mathrm{fin})$ con $\beta_\mathrm{fin}=4$ nos basamos en la figura \ref{fig:Msq-err-comp} para escogier los parámetros $dx=0.05$ y $\beta_\mathrm{ini} = 2^{-9}\beta_\mathrm{fin}=0.156$ 
\begin{figure}[!ht]
	\centering
