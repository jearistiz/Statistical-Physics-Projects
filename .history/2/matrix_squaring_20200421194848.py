...
def optimization(beta_fin=4, x_max=5, potential=harmonic_potential, potential_string='harmonic_potential',
    ...
    plt.figure()
    plt.contourf(DX,BETA_INI,error)
    plt.show()
    plt.close()
...
N_iter_max = 20
t_0 = time()
error, dx_grid, beta_ini_grid, dx_grid_test = optimization(4,5,harmonic_potential,'harmonic_potential',201,301,1,20)
t_0 = time()
print( 'Optimization:  beta_fin=%.3f,   x_max=%.3f,   potential=%s\nnx_min=%d,   nx_max=%d,   N_iter_min=%d,   N_iter_max=%d \n'\
