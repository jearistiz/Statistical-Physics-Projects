...
def path_naive_sampling(N_path = 10,beta = 4., n_steps = int(1e5), delta = 0.5, potential = harmonic_potential, append_every = 1):
    """
    Uso: 
    """
    dtau = beta/N_path
    ...
    t_1 = time()
    print('Path integral naive sampling:   %d iterations -> %.2E seconds'%(n_steps,t_1-t_0))
    pathss_x = np.array(pathss_x)
    return pathss_x

def figures_fn( pathss_x, beta = 4 , N_plot = 201, x_max = 3, n_steps=int(1e5), append_every=1,
    ...
    if show_path:
        ...
        if save_plot:
            plt.savefig(script_dir+'/'+file_name+'-path.eps')
        if show_plot:
            plt.show()
        plt.close()
    return 0
...
append_every = 1
pathss_x = path_naive_sampling()

script_dir = os.path.dirname(os.path.abspath(__file__)) #path completa para este script
msq_file = 'pi_x-harmonic_potential-x_max_5.000-nx_201-N_iter_7-beta_fin_4.000.csv'
...
figures_fn( pathss_x, beta = beta , N_plot = N_plot, x_max = x_max, n_steps=n_steps,
                N_beta_ticks = N_path+1, 
                msq_file=msq_file, file_name='path-plot-prueba', 
                show_path=True, show_matrix_squaring=True, save_plot=True, show_plot=True)