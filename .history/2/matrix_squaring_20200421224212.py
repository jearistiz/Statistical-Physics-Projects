...
def optimization(   beta_fin=4, x_max=5, potential=harmonic_potential, 
    ...
    if save_opt_data:
        if opt_data_file_name is None:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            opt_data_file_name =  '/pi_x-ms-opt-%s-beta_fin_%.3f-x_max_%.3f-nx_min_%d-nx_max_%d\
                                   -nx_sampling_%d-N_iter_min_%d-N_iter_max_%d.csv'\
                                   %(potential_string, beta_fin, x_max, nx_min, nx_max, 
                                    nx_sampling, N_iter_min, N_iter_max)
