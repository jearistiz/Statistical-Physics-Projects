...
def run_pi_x_squaring(x_max = 5., nx = 200, N_iter = 7, beta_fin = 4, potential = harmonic_potential, potential_string =  'harmonic_potential', print_steps=True):
    beta_ini = beta_fin * 2**(-N_iter)
    rho, grid_x, dx = rho_trotter(x_max, nx, beta_ini, potential)
    rho, trace_rho, beta_fin_2 = density_matrix_squaring(rho, grid_x, N_iter, beta_ini, print_steps)
    print('\n----------------------------------------------------------------',
            u'beta_fin = %.3f   Z(beta_fin) = Tr(rho(beta_fin)) ≈ %.3E'%(beta_fin_2,trace_rho))
    # Normalización de rho y cálculo de densidades de probabilidad para valores en x_grid
