...
def run_pi_x_sq_trotter(x_max=5., nx=201, N_iter=7, beta_fin=4, potential=harmonic_potential,
                        ...
                        plot=True, plot_QHO_theory=True, save_plot=True, show_plot=True,
                        plot_file_name=None):
    """
...
def average_energy(read_Z_data=True, generate_Z_data=False, Z_file_name = None,
    ...
    if plot_energy:
        ...
        plt.figure(figsize=(8,5))
        plt.plot(temp_read,E_avg,label=u'$\langle E \\rangle$ via matrix squaring')
        plt.plot(temp_read,E_QHO_avg_theo(beta_read),label=u'$\langle E \\rangle$ (QHO) teórico')
        plt.legend(loc='best')
        plt.xlabel(u'$T$')
...
def optimization(generate_opt_data=True, read_opt_data=False, beta_fin=4, x_max=5, 
                 ...
                 plot_opt=True, show_plot_opt=True, save_plot_opt=True, opt_plot_file_name=None,
                 print_summary=True):
    """
    ...
    Recibe:
        ...
        opt_data_file_name: str ->  nombre de archivo para datos de optimización. Si 
    ...
    if generate_opt_data:
        ...
        for N_iter in N_iter_values:
            ...
            for nx in nx_values:
                ...
                theoretical_Z = dx * np.sum(theoretical_pi_x)
                error_comp_theo = calc_error(pi_x,theoretical_pi_x,dx)
                row.append(error_comp_theo/theoretical_Z)
            error.append(row)

    elif read_opt_data:
            
        error =  pd.read_csv(opt_data_file_name, index_col=0, comment='#')
        dx_grid = error.columns.to_numpy(dtype=float)
        beta_ini_grid = error.index.to_numpy(dtype=float)
        error = error.to_numpy(dtype=float)
    ...
    if plot_opt:
        ...
        cbar = plt.colorbar(cp, use_gridspec=True,)
