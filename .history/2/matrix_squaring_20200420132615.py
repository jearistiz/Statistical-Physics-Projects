...
def save_csv(data, data_headers, file_name, relevant_info, print_data=True):
    """
    Uso: guarda datos de la distribución de probabilidad pi(x;beta) en un archivo .csv

    Recibe:
        data: numpy array of arrays, shape=(nx,ny)  ->  cada lista es una columna del archivo.
    ...
    # Almacena datos de probabilifad en diccionario: grid_x para posiciones y x_weights para
    # valores de densidad de probabilidad.
    if len(data_headers)!=len(data):
        data_headers = range(len(data))
        print('Nota: no hay suficientes headers en data_headers en save_csv().\nLos headers dados se cambian por números 0,1,...')
    data_dict = {}
