...
def C_QHO_canonical_ensemble(x,beta,plot=False,save=True,show=False):
    if plot==True:
        ...
        plt.legend(loc='best')
        if save==True:
            plt.savefig('CHO_finite_temp_beta_%d_%d.eps'%(beta,(beta-int(beta))*100))
        if show==True:
            plt.show()
        plt.close()
        return 0 
...
legend_loc = ['lower center','lower right','best']
