...
def metropolis(x0=0.0, delta_x=0.5, delta_steps=20, N=int(1e3), prob_sampling=[psi_0_1,canonical_ensemble_prob], beta=0.2):
    x_hist = [x0]
    n_hist = [0]
    for k in range(N):
...
for i in range(10):
    psi = add_energy_level(psi)
    grid_x.append(grid_x[-1]+delta_x)
    psi = add_x_value(psi,grid_x[-1])
