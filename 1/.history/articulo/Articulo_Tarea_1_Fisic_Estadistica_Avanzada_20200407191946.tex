...
\begin{equation}
	p(x_{old} \rightarrow x_{new}) = \min\left((1,|\psi_0(x_{new})/\psi_0(x_{old})|\right)	
\end{equation}
...
\begin{equation}
	p(x_{old} \rightarrow x_{new}) = \min\left((1,|\frac{\psi_{n_{old}}(x_{new})}{\psi_{n_{old}}(x_{old})}|^2\right).
\end{equation}
Además, para tener en cuenta los cambios en niveles de energía dados por el contacto con el baño térmico se propone en cada nueva iteración un $n_{new}=n_{old} + \Delta n$ donde los valores $\Delta n = \pm 1$ se escogen aleatoriamente con probabilidad $1/2$. La probabilidad de aceptación de este nuevo nivel de energía está determinada por el factor de Boltzmann
\begin{equation}
	p(n_{old} \rightarrow n_{new}) = \min\left(1,|\frac{\psi_{n_{new}}(x_{old})}{\psi_{n_{old}}(x_{old})}|^2 \exp[-\beta\left(E_{n_{new}}-E_{n_{old}}\right)]\right).
\end{equation}
