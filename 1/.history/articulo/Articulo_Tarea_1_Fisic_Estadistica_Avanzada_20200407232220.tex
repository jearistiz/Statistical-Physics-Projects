...
\subsection{Límite de muy baja temperatura\label{subsec:limite-T-0}}
...
En cuanto al tiempo de cómputo del algoritmo, éste depende del valor de $\beta$. Conforme $\beta$ disminuye (se considera mayores temperaturas), el tiempo de cómputo aumenta, ya que el sistema puede acceder a niveles de energía más altos. En el algoritmo ésto se traduce en que se deben computar los valores de $\psi_n$ para todos los valores de $x$ considerados hasta el momento, esto es, que hacen parte de la lista con la que se construye el histograma. En este punto tal vez el algoritmo pueda ser optimizado para reducir el tiempo de cómputo, ingeniando una manera de no tener que calcular $psi_n$ para todos los valores de $x$ considerados, sino solo para los que necesiten ser usados. En el caso de una máquina modelo 2011 (esto no es muy relevante, lo relevante realmente son las proporciones entre los tiempos para diferentes $\beta$) el algoritmo demora del orden de $180s$ para el caso $\beta = 0.2$, $150s$ para $\beta = 1.0$ y $90s$ para $\beta=5.0$. En el caso de $T=0$ tratado en la sección \ref{subsec:limite-T-0} el algoritmo demora del orden de $20s$. Es importante también mencionar que los algoritmos se corrieron en python3 v3.6.
