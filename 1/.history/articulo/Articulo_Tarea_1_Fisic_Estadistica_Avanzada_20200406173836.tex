...
En el caso clásico y el límite de muy baja temperatura ($T \rightarrow 0 $), según discutimos en la sección \ref{subsec:caso-clasico}, $\pi^{(C)}(x) \rightarrow \delta(x)$. En la figura (\hl{referenciar la figura}) encontramos $\pi^{(C)}(x;\beta)$ para diferentes valores de $\beta$, cada vez más cercanos a $0$ y podemos ver que la distribución tiende a una delta de Dirac, tal como se mencionó. 
\hl{Holaaa queé tal}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\linewidth]{figures/QHO_finite_temp_beta_1_00}
\caption{\hl{change caption}}
\label{fig:beta_1.0}
\end{figure}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\linewidth]{figures/QHO_finite_temp_beta_5_00}
\caption{\hl{change caption}}
\label{fig:beta_5.0}
\end{figure}
...
\section{Código 1: Oscilador Armónico Cuántico a muy baja temperatura \texorpdfstring{$T \rightarrow 0$}{T tendiendo a cero}}


\inputminted{python}{QHO_ground_state_metropolis.py}
\begin{minted}{python}
import numpy as np
