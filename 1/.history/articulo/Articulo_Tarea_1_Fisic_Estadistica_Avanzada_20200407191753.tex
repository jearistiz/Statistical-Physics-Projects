...
\begin{equation}
	p(x_{old} \rightarrow x_{new}) = \min\left((1,|\psi_{n_{old}}(x_{new})/\psi_{n_{old}}(x_{old})|^2\right).
\end{equation}
...
\begin{equation}
	p(n_{old} \rightarrow n_{new}) = \min\left(1,|\psi_{n_{new}}(x_{old})/\psi_{n_{old}}(x_{old})|^2\exp[-\beta\left(E_{n_{new}}-E_{n_{old}}\right)]\right).
\end{equation}
