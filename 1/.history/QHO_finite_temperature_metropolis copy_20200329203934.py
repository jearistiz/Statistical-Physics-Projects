...
def metropolis(x0=0.0, delta=0.5, N=int(1e3), prob_sampling=[psi,canonical_ensemble_prob], beta=0.2):
    x_hist = [x0]
    n_hist = [0]
    for k in range(N):
        ##### Spatial monte carlo P(x -> x')
        xnew = x_hist[-1] + np.random.choice(range(-20,20))*delta####################### +  AAAAADDDDDD CODE HERE
        try:
