...
def classical_theory(x,beta):
    return (beta/(2*np.pi))**0.5 * np.exp(-beta*x**2 / 2)

def quantum_theory(x,beta):
...
x_hist, n_hist = metropolis_finite_temp(x0=0.0, delta_x=delta_x, delta_steps=delta_steps, 
                            N=N_metropolis, prob_sampling=[psi,canonical_ensemble_prob], beta=0.2)
