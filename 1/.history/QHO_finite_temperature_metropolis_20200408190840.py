...
def metropolis_finite_temp(x0=0.0, delta_x=0.5, N=int(1e3), prob_sampling=[psi_0_1()[0],canonical_ensemble_prob], beta=5):
...
def CHO_canonical_ensemble(x,beta=5,plot=False,savefig=True,show=False):
    if plot==True:
        x = np.linspace(-3,3,201)
        plt.figure()
        pdf = (beta/(2.*np.pi))**0.5 * np.exp(-x**2*beta / 2.)
        plt.plot(x,pdf,label=u'$\\beta = %.1f$'%beta)
        plt.xlim(-3,3)
        plt.xlabel('$x$')
...
def run_metropolis(psi_0_1 = psi_0_1, x_limit = 5., N_points_x = 51,
    ...
    """
    # Inicializa autofunciones de energía en diccionario psi generado por función psi_0_1()
    psi, grid_x = psi_0_1(x_limit,N_points_x)
    
    # Se almacenan probabilidades en una lista:     la amplitud de probabilidad psi de las autofunciones
    #                                               y el factor de Boltzmann del ensamble canónico
    prob_sampling = [psi, canonical_ensemble_prob]

    ###### Se ejecuta algoritmo metropolis y se mide tiempo de cómputo
    t_0 = time()
    ...
    if plot==True:
        # Se grafica histograma para posiciones
        x_plot = np.linspace(-x_plot_0,x_plot_0,200)
        ...
        # Se grafica los histograma para niveles de energía
...
for i,beta in enumerate(beta_array):
    run_metropolis(N_metropolis=1e6,beta=beta,showplot=False,savefig=True)
...
for beta in beta_array_CHO:
    CHO_canonical_ensemble(0,beta=beta,plot=True,save=True)
