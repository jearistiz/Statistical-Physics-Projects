...
def CHO_canonical_ensemble(x,beta,plot=False,savefig=True,show=False):
    if plot==True:
...
def run_metropolis(psi_0_1 = psi_0_1, x_limit = 5., N_points_x = 51,
                    ...
                    plot=True, showplot = True, savefig = False, legend_loc = 'best', x_plot_0 = 6):
    """
    ...
    Recibe:
        ...
        plot: bool                         -> escoge si se grafica o no
    ...
    # Se grafican los histogramas para posiciones y niveles de energía
    if plot==True:
        ...
        plt.plot(n_plot,boltzmann_probability(n_plot+1/2))
        plt.xlabel(u'$n$')
...
for i,beta in enumerate(beta_array):
    run_metropolis(beta=beta,showplot=False,savefig=True)
...
### Test
# plt.hist(metropolis_finite_temp()[0],normed=True)
# plt.plot()
# plt.show()
