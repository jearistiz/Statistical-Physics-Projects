...
def metropolis(N=int(1e6),x0=0.0,delta=0.5,prob_amplitude_sampling=QHO_ground):
    ...
    N = int(N)
    for k in range(N):
        # Se propone nueva posición para x con distribución uniforme centrada en valor anterior
        xnew = x_hist[-1] + np.random.uniform(-delta,delta)
        # Se calcula probabilidad de aceptancia del algoritmo Metrópolis
...
def run_metropolis(N=1e5, x0=0.0, delta_x=0.5, prob_amplitude_sampling=QHO_ground, 
    ...
    N_plot: list                            -> número de valores de x para los que se grafica densidad de probabilidad
    """
    N = int(N)
