...
def psi_0_1(x_limit = 5, N_points_x = 101):  #creates first two energy eigenfunctions
    ...
    Recibe:
        x_limit: float      -> los valores de x serán N_points_x igualmente espaciados entre [-x_limit,x_limit]
        N_ponts_x: int      -> los valores de x serán N_points_x igualmente espaciados entre [-x_limit,x_limit]
    
    Devuelve:
        psi: dict           -> psi[x][n] corresponde a la autofucnión de energía \psi_{n}(x) n = 0,1.
...
def add_energy_level(psi):            #adds new energy eigenfunction to psi
    """
    Uso:    Recibe diccionario generado por fucnión psi_0_1 y entrega diccionario con autofunciones 
            con un nivel de energía adicional.

    Recibe:
    psi: dict       -> diccionario con autofunciones de energía psi[x][n] y máximo n = n_max = len(psi[0])
...
def add_x_value(psi,x):  #adds new x value to psi
    """
    Uso:    Recibe diccionario generado por fucnión psi_0_1 y entrega diccionario con autofunciones 
            con una posición adicional dada por el valor de x.

    Recibe:
    psi: dict       ->  diccionario con autofunciones de energía: psi[x][n] 
    
    Devuelve:
    psi: dict       ->  diccionario actualizado con nueva posición accesible x para todos los valores 
                        de n accesibles anteriormete.
    """
...
def boltzmann_probability(En,beta):
    ...
    Devuelve:
        probabilidad de encontrar el oscilador armónico cuántico en nivel de energía "En" a tmeperatura T.
    """
...
def metropolis_finite_temp(x0=0.0, delta_x=0.5, N=1e3, prob_sampling=[psi_0_1()[0],canonical_ensemble_prob], beta=5):
    ...
    Recibe: 
        ...
        prob_sampling[0]: dict      -> diccionario con autofunciones de energía generado por función psi_0_1().
        prob_sampling[1]: func      -> función que calcula factor de Boltzmann.
        beta: float                 -> inverso de temperatura en unidades reducidas beta = 1/T.


    Devuelve:
        x_hist: list    ->  lista con la que se calcula el histograma que aproxima la densidad de 
                            probabilidad de encontrar al oscilador armónico cuántico (en presencia 
                            de baño térmico) en una posición x.
        n_hist: list    ->  lista con la que se calcula el histograma que aproxima distribución de
                            Boltzmann para el caso del oscilador armónico cuántico.
        prob_sampling[0]: dict      ->  diccionatrio de autofunciones de energía actualizado para 
                                        todos los valores de x_hist y n_hist. Se accede a ellos mediante
                                        prob_sampling[0][x][n].
    """
...
def run_metropolis(psi_0_1 = psi_0_1, x_limit = 5., N_points_x = 51,
                    x0 = 0.0, delta_x = 0.5, N_metropolis = int(1e5),  canonical_ensemble_prob = canonical_ensemble_prob, beta = 5.,
    ...
    if plot==True:
        ...
        plt.plot(n_plot,boltzmann_probability(n_plot+1/2,beta),'o-',label=u'$e^{-\\beta E_n}/Z(\\beta)$')
        plt.xlabel(u'$n$')
...
# Corremos algoritmo para gráfica de límite de baja temperatura en el caso clásico
